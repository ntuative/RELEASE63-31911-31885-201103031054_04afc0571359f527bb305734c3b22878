package com.sulake.habbo.widget.roomchat
{
   ...
   public class RoomChatItem
   {
      ...
      private static const const_918:Number = 18;
      
      private static const const_590:int = 6;
      
      private static const const_1251:int = 6;
      
      private static const const_919:int = 35;
      
      private static var var_326:IRegionWindow;
      
      private static var var_387:IRegionWindow;
      ...
      public function RoomChatItem(param1:RoomChatWidget, param2:IHabboWindowManager, param3:IAssetLibrary, param4:String, param5:IHabboLocalizationManager, param6:String)
      {
         ...
         if(!var_387)
         {
            var_387 = this._windowManager.buildFromXML(this._assetLibrary.getAssetByName("bubble_shout").content as XML,1) as IRegionWindow;
            var_387.tags.push("roomchat_bubble");
            var_387.x = 0;
            var_387.y = 0;
            var_387.width = 0;
            var_387.background = true;
            var_387.mouseThreshold = 0;
            var_204.setParamFlag(WindowParam.WINDOW_PARAM_FORCE_CLIPPING,true);
         }
         if(!var_388)
         {
      ...
      {
         this.var_1286 = param1.chatType;
         this.var_1028 = param1.userId;
         this.var_838 = param1.userName;
         this.var_1027 = param1.userCategory;
         this._message = param1.text;
         this.var_460 = param1.links;
         this.var_1570 = param1.userX;
         this.var_228 = param1.userImage;
         this.var_2093 = param1.userColor;
      ...
      public function set y(param1:Number) : void
      {
         ...
         if(this._window != null)
         {
            this._window.y = param1 - this.var_676;
         }
      }
      ...
      {
         var _loc5_:Rectangle = new Rectangle(this._x,this.var_153,this.width,const_918);
      ...
      public function renderView() : void
      {
         ...
         switch(this.var_1286)
         {
            ...
            case RoomWidgetChatUpdateEvent.const_112:
               this._window = var_387.clone() as IRegionWindow;
               break;
         ...
         if(this.var_228 && !isRespect)
         {
            ...
            if(userIconWindow)
            {
               ...
               userIconWindow.x = x1;
               userIconWindow.y = y1;
               this.var_676 = Math.max(0,(this.var_228.height - background.height) / 2);
         ...
         if(nameWindow != null)
         {
            if(!isRespect)
            {
               nameWindow.text = this.var_838 + ": ";
               nameWindow.y += this.var_676;
               nameWindow.width = nameWindow.textWidth + const_590;
            }
            else
            {
               nameWindow.text = "";
         ...
         if(this.var_1286 == RoomWidgetChatUpdateEvent.const_284)
         {
            textWindow.text = this.var_76.registerParameter("widgets.chatbubble.respect","username",this.var_838);
            this._width = const_919;
         }
         else if(this.var_1286 == RoomWidgetChatUpdateEvent.const_285)
         {
            textWindow.text = this.var_76.registerParameter("widget.chatbubble.petrespect","petname",this.var_838);
            this._width = const_919;
         }
         else if(this.var_460 == null)
         {
            textWindow.text = this.message;
         }
         else
         {
            this.var_675 = new Array();
            lastLinkEndPos = -1;
            i = 0;
            while(i < this.var_460.length)
            {
               filteredLink = this.var_460[i][1];
               placeHolder = "{" + i + "}";
               placeholderPos = this._message.indexOf(placeHolder);
               lastLinkEndPos = placeholderPos + filteredLink.length;
               this.var_675.push([placeholderPos,lastLinkEndPos]);
            ...
            i = 0;
            while(i < this.var_675.length)
            {
               linkPos = this.var_675[i];
               try
               {
         ...
         if(textWindow.visible)
         {
            ...
            if(nameWindow != null)
            {
               ...
               if(nameWindow.width > const_590)
               {
                  textWindow.x -= const_590 - 1;
               }
            }
            textWindow.y += this.var_676;
            messageWidth = textWindow.textWidth;
            textWindow.width = messageWidth + const_1251;
         ...
         if(pointerWindow != null && pointerWindow.visible)
         {
            ...
            pointerWindow.y += this.var_676;
         }
         var bitmap:BitmapData = this.buildBubbleImage(nameWindow.width + textWindow.width,background.height,this.var_2093,isRespect);
         this._window.width = bitmap.width;
         this._window.y -= this.var_676;
         this._width = this._window.width;
         background.bitmap = bitmap;
         background.y = this.var_676;
      }
      ...
      private function onMessageLinkMouseClick(param1:WindowMouseEvent) : void
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < this.var_675.length)
         {
            if(_loc2_ >= this.var_675[_loc3_][0] && _loc2_ <= this.var_675[_loc3_][1])
            {
               if(this.var_460[_loc3_][2] == 0)
               {
                  HabboWebTools.openExternalLinkWarning(this.var_460[_loc3_][0]);
               }
               else if(this.var_460[_loc3_][2] == 1)
               {
                  HabboWebTools.openWebPage(this.var_1569 + this.var_460[_loc3_][0],"habboMain");
               }
               else
               {
                  HabboWebTools.openWebPage(this.var_1569 + this.var_460[_loc3_][0]);
               }
            }
            _loc3_++;
         }
      ...
      {
         this._widget.onItemMouseClick(this.var_1028,this.var_838,this.var_1027,this._roomId,this._roomCategory,param1);
      }
      
      private function onBubbleMouseDown(param1:WindowMouseEvent) : void
      {
         this._widget.onItemMouseDown(this.var_1028,this.var_1027,this._roomId,this._roomCategory,param1);
      }
      
      private function onBubbleMouseOver(param1:WindowMouseEvent) : void
      {
         this._widget.onItemMouseOver(this.var_1028,this.var_1027,this._roomId,this._roomCategory,param1);
      }
      
      private function onBubbleMouseOut(param1:WindowMouseEvent) : void
      {
         this._widget.onItemMouseOut(this.var_1028,this.var_1027,this._roomId,this._roomCategory,param1);
      }
