   ...
   public class ClubBuyItem
   {
      ...
      public function ClubBuyItem(param1:ClubBuyOfferData, param2:ICatalogPage)
      {
         ...
         if(param1.vip)
         {
            if(param1.upgrade)
            {
               _loc3_ = this.method_4("club_buy_vip_upgrade_item");
            }
            else
            {
               _loc3_ = this.method_4("club_buy_vip_item");
            }
         }
         else
         {
            _loc3_ = this.method_4("club_buy_hc_item");
         }
         this._window = this.var_425.viewer.catalog.windowManager.buildFromXML(_loc3_) as IWindowContainer;
      ...
      {
         var _loc2_:ClubBuyController = (this.var_425.viewer.catalog as HabboCatalog).getClubBuyController();
         (this.var_425.viewer.catalog as HabboCatalog).showPurchaseConfirmation(this.var_91,this.var_425.pageId);
      }
      
      private function method_4(param1:String) : XML
      {
         if(!this.var_425 || !this.var_425.viewer || !this.var_425.viewer.catalog || !this.var_425.viewer.catalog.assets)
         {
            return null;
         }
         var _loc2_:XmlAsset = this.var_425.viewer.catalog.assets.getAssetByName(param1) as XmlAsset;
         if(_loc2_ == null)
         {
            return null;
         }
