package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_476 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_476 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2242 = _loc3_.localizationId;
         this.var_360 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_476,this);
               if(_loc5_)
               {
                  this.var_360 = _loc5_.colors;
         ...
         this._offers.add(this.var_476,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_756,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_360)
         {
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_57)
         {
            ...
            if(this.var_196 == null || _loc3_ >= this.var_196.length)
            {
               return;
            }
            this.var_758 = _loc3_;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_502 < 0 || this.var_502 > this.var_360.length)
         {
            this.var_502 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2242)
         {
            this.var_196 = this.parseSellableBreeds(param1.sellableBreeds);
            this.var_758 = 0;
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_476 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         var _loc2_:Array = new Array();
         var _loc3_:int = 0;
         while(_loc3_ < this.var_196.length)
         {
            _loc5_ = this.var_196[_loc3_] as int;
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_476,_loc5_),this.getRaceLocalizationKey(this.var_476,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_196 == null || this.var_758 >= this.var_196.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_502 >= 0 && this.var_502 < this.var_360.length)
         {
            _loc7_ = this.var_360[this.var_502] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_196[this.var_758] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_476,_loc3_,_loc2_,AvatarScaleType.const_56,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_296,AvatarAction.const_792);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_502 >= this.var_360.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_360[this.var_502] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_196[this.var_758] as int;
      ...
      private function updateAvailableBreeds(param1:String) : void
      {
         ...
         if(this.var_196 == null)
         {
            ...
            this.var_196 = this.parseSellableBreeds(_loc2_);
            this.var_758 = 0;
            if(this.var_196 != null)
            {
               this.updateBreedSelections();
