package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_419:IFriendCategoriesDeps;
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_343);
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            {
               this.method_6(removedFriendId,true);
            }
            status = 3;
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_419.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_419.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_419.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_419.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_419.view.setNewMessageArrived();
                  this.var_419.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
               wasSelected = this.method_6(updatedFriend.id,true);
            ...
            for each(addedFriend in p.addedFriends)
            {
               ...
               this.method_6(addedFriend.id,true);
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_343).received = true;
         this.findCategory(FriendCategory.const_1069).received = true;
         ...
         for each(_loc3_ in this.getCategoriesNotReceived())
         {
            ...
            {
               Util.remove(this.var_96,_loc3_);
               _loc3_.dispose();
            }
         }
      }
      
      private function method_6(param1:int, param2:Boolean) : Boolean
      {
         var _loc4_:* = null;
         var _loc5_:* = null;
         if(param2)
