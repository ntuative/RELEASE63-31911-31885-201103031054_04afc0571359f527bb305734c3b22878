package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_859:int = 3;
      
      private var var_395:AssetLoaderStruct;
      
      private var var_858:Timer;
      ...
      private const const_1976:int = 100;
      
      private const const_1447:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_395 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_858 = new Timer(this.const_1976,1);
         this.var_858.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_395)
         {
            this.var_395.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_395.addEventListener(AssetLoaderEvent.const_45,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_395)
         {
            this.var_395.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_395.removeEventListener(AssetLoaderEvent.const_45,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_859);
            if(this.var_1301.indexOf("?") > 0)
            {
               _loc2_ = this.var_1301 + "&retry=" + this.var_859;
            }
            else
            {
               _loc2_ = this.var_1301 + "?retry=" + this.var_859;
            ...
            this.var_395 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2143,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1041[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1041[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1592)
         {
            this.var_1042.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_523[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_523[_loc3_] = _loc6_;
            }
            this.var_860[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_860)
         {
            _loc7_ = true;
            _loc4_ = this.var_860[_loc3_];
      ...
      {
         var _loc1_:* = null;
         while(this.var_685.length > 0 && this.var_686.length < this.const_1447)
         {
            _loc1_ = this.var_685[0];
            _loc1_.startDownloading();
            this.var_686.push(this.var_685.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_685.indexOf(param1) == -1 && this.var_686.indexOf(param1) == -1)
         {
