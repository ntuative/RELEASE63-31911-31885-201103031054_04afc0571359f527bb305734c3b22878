package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_358:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_358 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_615 && this.var_1430)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_92,this.var_953,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_34,parseInt(FigureData.const_752));
               _loc1_ = _loc5_.getCroppedImage(AvatarSetType.const_34);
               _loc5_.dispose();
            }
         }
         else
         ...
         if(this.var_358)
         {
            if(this.var_358.bitmap)
            {
               this.var_358.bitmap.dispose();
            }
            this.var_358.bitmap = new BitmapData(this.var_358.width,this.var_358.height,true,0);
            _loc7_ = (this.var_358.width - _loc1_.width) / 2;
            _loc8_ = (this.var_358.height - _loc1_.height) / 2;
            this.var_358.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_615 = this._controller.figureData.getFigureString();
               this.var_953 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1821,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_615)
               {
                  this._controller.loadAvatarInEditor(this.var_615,this.var_953,this._controller.clubMemberLevel);
               }
         }
      }
