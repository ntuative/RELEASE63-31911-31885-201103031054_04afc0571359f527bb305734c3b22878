package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_588:Rectangle;
      
      private var var_1672:int = 0;
      
      private var var_408:IAvatarRenderManager;
      
      private var var_657:Boolean;
      
      private var var_1673:BitmapData;
      
      private const const_1972:int = 13421772;
      
      private const const_1973:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_645 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1672 = Math.max(this.var_1672,_loc6_.colorLayerIndex);
            }
         }
         this.var_408 = this.var_65.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_645 != null && !this.var_1671)
            {
               _loc3_ = this.var_645;
            }
            else
            ...
            if(this.var_723)
            {
               switch(this.var_723.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_40:
                     _loc2_.style = HabboIconType.const_720;
                     break;
                  case HabboClubLevelEnum.const_43:
                     _loc2_.style = HabboIconType.const_787;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1972;
         }
         else
         {
            this._window.color = this.const_1973;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_408.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_408.isFigureReady(_loc1_))
         {
            this.var_408.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_332[_loc6_] + "_" + FigureData.const_152;
               _loc4_ = this.var_408.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_588 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1673)
               {
                  ...
                  this.var_1673 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_588.width,this.var_588.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_588.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_588.y;
               if(this.var_1671 && _loc2_.colorLayerIndex > 0)
               {
