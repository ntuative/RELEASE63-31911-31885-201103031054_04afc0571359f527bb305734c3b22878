package com.sulake.habbo.quest
{
   ...
   public class QuestController implements IDisposable
   {
      ...
      private var var_296:QuestCampaign;
      ...
      public function activateQuest(param1:Quest) : void
      {
         if(param1)
         {
            this.var_87.communication.getHabboMainConnection(null).send(new ActivateQuestMessageComposer(param1.id));
         }
      }
      ...
      {
         if(this.var_636 != MODE_TABS)
         {
            this.var_636 = MODE_NO_TABS;
         }
         this.detachCampaign();
         ...
         this.var_296 = _loc4_;
         this.var_296.openCampaign(param3);
         this.prepareWindow();
         this.attachCampaign();
         if(this.var_636 == MODE_TABS)
         {
      ...
      private function attachCampaign() : void
      {
         ...
         this.var_1174.addChild(this.var_296.window);
         var _loc1_:String = "quest_" + this.var_296.code;
      ...
      private function updateTabs() : void
      {
         ...
         if(this.var_296)
         {
            _loc1_ = this.var_296.code;
         }
         else if(this.var_87.defaultCampaignCode.length > 0)
         {
            _loc1_ = this.var_87.defaultCampaignCode;
         }
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_87.getXmlWindow(_loc1_));
         this._window.findChildByTag("close").procedure = this.onWindowClose;
         this._window.center();
         this.var_1174 = this._window.findChildByName("content") as IWindowContainer;
      }
      ...
      private function refreshTabs() : void
      {
         ...
         for each(_loc4_ in this.var_159.getKeys())
         {
            ...
            if(!_loc2_)
            {
               ...
               _loc2_ = this.var_87.windowManager.createWindow("tab_" + _loc4_,"${" + _loc6_ + "}",WindowType.const_495) as ITabButtonWindow;
               if(_loc3_.expired)
               {
      ...
      private function onTabSelect(param1:WindowEvent, param2:IWindow = null) : void
      {
         ...
         this.var_87.getQuestCampaign(_loc3_);
      }
   }
}
