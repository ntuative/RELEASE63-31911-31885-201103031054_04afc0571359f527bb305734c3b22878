package com.sulake.habbo.localization
{
   ...
   public class HabboLocalizationManager extends CoreLocalizationManager implements IHabboLocalizationManager
   {
      ...
      private static var var_1280:String = "%";
      ...
      public function HabboLocalizationManager(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null)
      {
         ...
         this.var_2137 = ["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII","XIII","XIV","XV","XVI","XVII","XVIII","XIX","XX"];
         super(param1,param2,param3);
         this.var_1586 = (param2 & 0) > 0;
         if(!this.var_1586)
         {
            lock();
         }
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1280);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1280,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         if(param2 != null)
         {
            ...
            if(!this.var_1586)
            {
               if("false" == this.var_168.getKey("use.default.localizations","false"))
               {
                  _loc5_ = this.var_168.getKey("external.texts.txt","external_texts.txt");
                  events.addEventListener(LocalizationEvent.const_165,this.onLocalizationLoaded);
                  super.loadLocalizationFromURL(_loc5_);
               }
            }
            _loc4_ = 1;
            while(this.var_168.keyExists("localization." + _loc4_))
            {
               _loc6_ = this.var_168.getKey("localization." + _loc4_,"");
               _loc7_ = this.var_168.getKey("localization." + _loc4_ + ".name","");
               _loc8_ = this.var_168.getKey("localization." + _loc4_ + ".url","");
      ...
      {
         var _loc2_:* = null;
         events.removeEventListener(LocalizationEvent.const_165,this.onLocalizationLoaded);
         if(this.var_168.keyExists("external.override.texts.txt"))
         {
            _loc2_ = this.var_168.getKey("external.override.texts.txt");
            events.addEventListener(LocalizationEvent.const_165,this.onLocalizationOverrideLoaded);
      ...
      {
         events.removeEventListener(LocalizationEvent.const_165,this.onLocalizationOverrideLoaded);
         unlock();
      }
      ...
      {
         return this.var_2137[Math.max(0,param1 - 1)];
      }
   }
}
