package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1069:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1648 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1070 = this.var_1648.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1070)
         {
            this.var_1070.procedure = this.onButtonClick;
         }
         this.var_1069 = this.var_1648.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1069)
         {
            this.var_1069.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_238 >= this.var_76.length - 1)
         {
            this.var_1069.disable();
         }
         else
         {
            this.var_1069.enable();
         }
         if(this.var_238 <= 0)
         {
            this.var_1070.disable();
         }
         else
         {
            this.var_1070.enable();
         }
      }
      ...
      {
         ++this.var_238;
         this.var_238 = Math.min(this.var_238,this.var_76.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_238;
         this.var_238 = Math.max(this.var_238,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_78.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_76.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_76 || this.var_76.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_76[this.var_238][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_76[this.var_238][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_76[this.var_238][1]).value;
