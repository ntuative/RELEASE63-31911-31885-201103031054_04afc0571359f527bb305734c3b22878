package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2398 = new RoomThumbnailRenderer(param1);
         this.var_1161 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         this.refreshRoomName(this.var_177,this.var_114);
         if(this.var_114.ownerName != "" && this.var_114.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_177,!!this.var_114.event ? this.getEventCtg(this.var_114.categoryId) : this.getRoomCtg(this.var_114.categoryId));
         this.refreshRoomDesc(this.var_177,this.var_114);
         this.refreshExtraCont();
         this.refreshInfo(this.var_177,"trading_allowed",this.var_114.allowTrading);
         this.refreshInfo(this.var_177,"doormode_doorbell",this.var_114.doorMode == 1);
         this.refreshInfo(this.var_177,"doormode_password",this.var_114.doorMode == 2);
      ...
      {
         this.refreshTextWithCaption("roomowner",this.var_177,this.var_114.ownerName);
      ...
      private function refreshExtraCont() : void
      {
         ...
         this.var_1161.refreshTags(_loc1_,this.var_114.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_114.score);
         if(this.var_114.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_114.eventCreationTime);
         }
         this.var_2398.refreshThumbnail(_loc1_,this.var_114.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_229,Tabs.const_212,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
