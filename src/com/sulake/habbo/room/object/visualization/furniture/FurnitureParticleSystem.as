package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_95:FurnitureParticleSystemEmitter;
      
      private var var_83:BitmapData;
      
      private var var_236:IRoomObjectSprite;
      ...
      private var var_1062:Number = 1.0;
      
      private var var_579:BitmapData;
      
      private var var_711:ColorTransform;
      
      private var var_1323:ColorTransform;
      
      private var var_1635:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_775:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_711 = new ColorTransform();
         this.var_711.alphaMultiplier = 1;
         this.var_1323 = new ColorTransform();
         this.var_1635 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_95 = this.var_1061[param1];
         this.var_880 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1634 >= 0)
         {
            this.var_236 = this._visualization.getSprite(this.var_1634);
            if(this.var_236 && this.var_236.asset)
            {
               if(this.var_236.width <= 1 || this.var_236.height <= 1)
               {
                  return;
               }
               if(this.var_83 == null)
               {
                  this.var_83 = this.var_236.asset.clone();
                  if(this.var_711.alphaMultiplier != 1)
                  {
                     this.var_579 = new BitmapData(this.var_83.width,this.var_83.height,true,4278190080);
                  }
               }
               this.var_2238 = -this.var_236.offsetX;
               this.var_2237 = -this.var_236.offsetY;
               this.var_236.asset = this.var_83;
            }
            if(this.var_83)
            {
            ...
            if(this.var_579)
            {
               this.var_579.fillRect(this.var_579.rect,4278190080);
            }
         }
      }
      ...
      {
         if(this.var_95 && this.var_95.roomObjectSpriteId == param3)
         {
            return this.var_95.y * this.var_1062;
         }
         return 0;
      }
      ...
      public function controlsSprite(param1:int) : Boolean
      {
         if(this.var_95)
         {
            return this.var_95.roomObjectSpriteId == param1;
         }
         return false;
      }
      ...
      {
         if(!this.var_95 || !this.var_236)
         {
            return;
         }
         if(this.var_83 && this.var_236.asset != this.var_83)
         {
            this.var_236.asset = this.var_83;
         }
         if(this.var_880)
         {
            if(this.var_95.roomObjectSpriteId >= 0)
            {
               this._visualization.getSprite(this.var_95.roomObjectSpriteId).visible = false;
            }
         }
      }
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_880 && this.var_95.hasIgnited)
         {
            this.var_880 = true;
         }
         _loc8_ = this.var_1157 * this.var_1062;
         this.var_95.update();
         if(this.var_880)
         {
            if(this.var_95.roomObjectSpriteId >= 0)
            ...
            {
               this.var_83.draw(this.var_579,this.var_1635,this.var_711,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_95.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2238 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1062;
               _loc4_ = this.var_2237 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1062;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1323.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_83.draw(_loc7_,this._translationMatrix,this.var_1323,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2239 = parseInt(param1.@size);
         this.var_1634 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1157 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1062 = this.var_2239 / 64;
         this.var_775 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_775 = Math.min(this.var_775,1);
         this.var_711.alphaMultiplier = this.var_775;
