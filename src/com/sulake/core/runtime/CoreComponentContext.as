package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_124:uint = 3;
      
      private static const const_1325:String = "asset-libraries";
      
      private static const const_1324:String = "library";
      
      private static const const_1321:String = "service-libraries";
      
      private static const const_1326:String = "library";
      
      private static const const_1319:String = "component-libraries";
      
      private static const const_1318:String = "library";
      
      private static const const_1322:String = "error_data";
      
      private static const const_1320:String = "error_cat";
      
      private static const const_1323:String = "error_desc";
       
      
      private var var_350:LibraryLoaderQueue;
      
      private var var_735:IEventDispatcher;
      
      private var var_488:uint;
      
      private var var_414:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_446 = param1;
         var _loc3_:int = 0;
         while(_loc3_ < CoreComponentContext.const_124)
         {
            this.var_415.push(new Array());
            this.var_599.push(0);
            _loc3_++;
         }
         ...
         switch(param2 & 0)
         {
            case Core.const_1152:
               ...
               this.var_414 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_707:
               debug("Core; using complex frame update handler");
               this.var_414 = this.complexFrameUpdateHandler;
               break;
            case Core.const_1557:
               debug("Core; using profiler frame update handler");
               this.var_414 = this.profilerFrameUpdateHandler;
               this.var_248 = new Profiler(this);
               attachComponent(this.var_248,[new IIDProfiler()]);
               var_446.addChild(new ProfilerViewer(this.var_248));
               break;
            case Core.const_683:
               debug("Core; using experimental frame update handler");
               this.var_414 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_414 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_350.length;
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            while(_loc3_ < CoreComponentContext.const_124)
            {
               _loc2_ = this.var_415[_loc3_] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1322,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1320,String(param3));
         ErrorReportStorage.setParameter(const_1323,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_735 = loadingEventDelegate;
         if(this.var_350 == null)
         {
            this.var_350 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1325)[0];
            if(node != null)
            {
               list = node.child(const_1324);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_326,e);
         }
         try
         {
            node = config.child(const_1321)[0];
            if(node != null)
            {
               list = node.child(const_1326);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_326,e);
         }
         try
         {
            node = config.child(const_1319)[0];
            if(node != null)
            {
               list = node.child(const_1318);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_326,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1571);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_735)
         {
            this.var_735.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_735)
         {
            ...
            this.var_735.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_735)
                  {
                     this.var_735.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_488 - this.var_350.length,this.var_488,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         param2 = param2 >= CoreComponentContext.const_124 ? 4294967295 : uint(param2);
         var _loc3_:Array = this.var_415[param2] as Array;
         if((this.var_1393 & 0) == Core.const_683)
         {
            _loc3_.push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            _loc3_.push(param1);
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc5_:* = null;
         var _loc4_:int = 0;
         while(_loc4_ < CoreComponentContext.const_124)
         {
            _loc3_ = this.var_415[_loc4_] as Array;
            if((this.var_1393 & 0) == Core.const_683)
            {
               for each(_loc5_ in _loc3_)
               {
                  if(_loc5_.receiver == param1)
                  {
                     _loc5_.dispose();
                     return;
                  }
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_414(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         criticality = 0;
         while(criticality < CoreComponentContext.const_124)
         {
            this.var_599[criticality] = 0;
            receivers = this.var_415[criticality];
            index = 0;
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         priority = 0;
         while(priority < CoreComponentContext.const_124)
         {
            t = getTimer();
            skip = false;
            if(t - msCurrentTime > maxTimePerFrame)
            {
               if(this.var_599[priority] < priority)
               {
                  ++this.var_599[priority];
                  skip = true;
               }
            }
            if(!skip)
            {
               this.var_599[priority] = 0;
               receivers = this.var_415[priority];
               index = 0;
               length = receivers.length;
               while(index != length)
               {
...
class UpdateDelegate implements IDisposable
{
   ...
   function UpdateDelegate(param1:IUpdateReceiver, param2:IContext, param3:int)
   {
      ...
      if(param2 && param1)
      {
         ...
         param2.displayObjectContainer.stage.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         this._lastUpdateTimeMs = getTimer();
      }
   }
   ...
   public function dispose() : void
   {
      ...
      {
         this._receiver = null;
         this._context.displayObjectContainer.stage.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         this._context = null;
      }
   }
   ...
   private function onEnterFrame(param1:Event) : void
   {
      ...
      if(!this.disposed)
      {
         ...
         try
         {
            this._receiver.update(msCurrentTime - this._lastUpdateTimeMs);
         }
         catch(e:Error)
         {
