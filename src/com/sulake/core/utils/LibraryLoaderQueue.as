package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1447:int = 4;
      ...
      {
         return this.var_411.length + this.var_270.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_270.length < const_1447 && this.var_411.length > 0)
            {
               _loc1_ = this.var_411.shift();
               this.var_270.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_411.indexOf(loader);
            if(index > -1)
            {
               this.var_411.splice(index,1);
            }
            index = this.var_270.indexOf(loader);
            if(index > -1)
            {
               this.var_270.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_411)
            {
               if(param2)
               {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_270)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
